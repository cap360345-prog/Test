--[[ -------------------------------------------------
    RealCorp Hub - Fixed & Enhanced ESP
    ------------------------------------------------- ]]
...
-- Settings
local SETTINGS_FILE = "RealCorpHub_Settings.json"
local settings = {
    autoLoad = false,
    autoSave = false,
    ...
    if resp then
        if resp.StatusCode == 200 then
            ...
            if ok and type(data) == 'table' then 
                ...
                warn("RealCorp Hub (Brainrot Finder): Failed to decode JSON or data is not a table. Error/Data: ", data)
            end
        else
            warn("RealCorp Hub (Brainrot Finder): HTTP request failed with status code: " .. resp.StatusCode)
        end
    else
        warn("RealCorp Hub (Brainrot Finder): No HTTP request function found or request returned nil.")
    end
    return {}
end
...
local function tryAutoJoin(servers)
    ...
    if maxAttempts > 0 and #filtered > 0 then
        for attempt = 1, maxAttempts do
            ...
            for _, server in ipairs(filtered) do
                if autoJoinMode == nil then return end
                print("RealCorp Hub: [Attempt " .. attempt .. "/" .. maxAttempts .. "] Trying JobId: " .. clean(server.jobId or server.instanceId))
                doTeleport(server)
                task.wait(0.05)
            end
        end
    ...
    for _, instance in ipairs(workspace:GetDescendants()) do
        ...
        if instance:IsA("Model") and brainrotDict[instance.Name] and not reportedPetsThisSession[instance] then
            ...
            if webhookURL then
                ...
                local payload = {
                    username = "RealCorp Hub Notifier",
                    embeds = {
                        {
                ...
                task.spawn(function()
                    ...
                    if not success then
                        warn("RealCorp Hub: Webhook failed.", response)
                    end
                end)
...
-- Create UI
 function createUI()
    ...
    brandLogo.Image = "rbxassetid://121939630888996"
    ...
    brandTitlePrimary.Text = "RealCorp Hub"
    ...
    sjJoinButton.Activated:Connect(function()
        ...
        if jobId ~= "" then
            ...
            if not success then
                warn("RealCorp Hub (Server Joiner): Teleport failed -", err)
                pcall(function()
                    StarterGui:SetCore("SendNotification", {
...
print("RealCorp Hub loaded successfully!")
